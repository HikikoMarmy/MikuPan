...
static SDL_GPUPrimitiveType PrimitiveFromGL(unsigned int mode)
{
    switch (mode)
    {
        case GL_TRIANGLE_STRIP: return SDL_GPU_PRIMITIVETYPE_TRIANGLESTRIP;
        case GL_LINES:          return SDL_GPU_PRIMITIVETYPE_LINELIST;
        case GL_LINE_LOOP:
        case GL_LINE_STRIP:     return SDL_GPU_PRIMITIVETYPE_LINESTRIP;
        case GL_POINTS:         return SDL_GPU_PRIMITIVETYPE_POINTLIST;
        case GL_TRIANGLES:
...
static SDL_GPUCompareOp CompareFromGL(unsigned int func)
{
    switch (func)
    {
        case GL_ALWAYS: return SDL_GPU_COMPAREOP_ALWAYS;
...
static void CreateFallbackTexture(void)
{
    ...
    SDL_GPUTransferBuffer *transfer = SDL_CreateGPUTransferBuffer(g_device, &tb);
    if (transfer == NULL)
    {
        return;
    }
...
SDL_GPUDevice *MikuPan_GPUGetDevice(void) { return g_device; }
SDL_GPUCommandBuffer *MikuPan_GPUGetCommandBuffer(void) { return g_cmd; }
SDL_GPURenderPass *MikuPan_GPUGetRenderPass(void) { return g_pass; }
SDL_GPUTextureFormat MikuPan_GPUGetSwapchainFormat(void) { return g_swapchain_format; }
...
unsigned int MikuPan_GPUGetSceneTextureId(void) { return g_scene_texture_id; }
unsigned int MikuPan_GPUGetSceneWidth(void) { return (unsigned int)g_scene_width; }
unsigned int MikuPan_GPUGetSceneHeight(void) { return (unsigned int)g_scene_height; }
                                ...
                                unsigned char *out_rgba)
{
    ...
    info.size = size;
    SDL_GPUTransferBuffer *transfer = SDL_CreateGPUTransferBuffer(g_device, &info);
    if (transfer == NULL)
    {
        return 0;
    }
                            ...
                            int height)
{
    ...
    SDL_CopyGPUTextureToTexture(
        ...
        (Uint32)width, (Uint32)height, 1, true);
    SDL_EndGPUCopyPass(copy);
}
...
int MikuPan_GPUGetCurrentShader(void) { return g_current_shader; }
...
static SDL_GPUGraphicsPipeline *GetPipeline(unsigned int primitive)
{
    ...
    blend.dst_color_blendfactor =
        g_state.additive_blend ? SDL_GPU_BLENDFACTOR_ONE :
    ...
    info.depth_stencil_state.enable_depth_test = g_state.depth_test ? true : false;
    info.depth_stencil_state.enable_depth_write = g_state.depth_write ? true : false;
    info.depth_stencil_state.compare_op = g_state.compare;
    info.target_info.color_target_descriptions = &color_desc;
    info.target_info.num_color_targets = 1;
    info.target_info.has_depth_stencil_target = g_target_has_depth ? true : false;
    ...
    if (pipeline == NULL)
    {
        info_log("SDL_CreateGPUGraphicsPipeline failed: shader=%d vao=%u err=%s",
...
static void BeginTargetPassIfNeeded(void)
{
    ...
    SDL_GPUViewport vp = {
        ...
        (float)g_viewport[2], (float)g_viewport[3],
        0.0f, 1.0f
    };
...
void MikuPan_GPUSetDepthWrite(int enabled) { g_state.depth_write = enabled ? 1 : 0; }
void MikuPan_GPUSetDepthFunc(unsigned int gl_func) { g_state.compare = CompareFromGL(gl_func); }
void MikuPan_GPUSetBlend(int enabled, int additive)
{
    g_state.blend = enabled ? 1 : 0;
    g_state.additive_blend = additive ? 1 : 0;
}
void MikuPan_GPUSetCullBack(void) { g_state.cull_mode = SDL_GPU_CULLMODE_BACK; }
void MikuPan_GPUSetCullFront(void) { g_state.cull_mode = SDL_GPU_CULLMODE_FRONT; }
void MikuPan_GPUSetCullNone(void) { g_state.cull_mode = SDL_GPU_CULLMODE_NONE; }
void MikuPan_GPUSetColorWriteMask(int r, int g, int b, int a)
{
    g_state.color_mask = 0;
    if (r) g_state.color_mask |= SDL_GPU_COLORCOMPONENT_R;
    if (g) g_state.color_mask |= SDL_GPU_COLORCOMPONENT_G;
    if (b) g_state.color_mask |= SDL_GPU_COLORCOMPONENT_B;
    if (a) g_state.color_mask |= SDL_GPU_COLORCOMPONENT_A;
...
void MikuPan_GPUSetViewport(int x, int y, int w, int h)
{
    ...
    if (g_pass != NULL)
    {
        SDL_GPUViewport vp = {(float)x, (float)y, (float)w, (float)h, 0.0f, 1.0f};
        SDL_SetGPUViewport(g_pass, &vp);
    }
}
...
void MikuPan_GPUSetTarget(MikuPan_GPUTarget target, int clear)
{
    ...
    if (target == MIKUPAN_GPU_TARGET_WINDOW)
    {
        if (g_swapchain == NULL)
        {
            SDL_WaitAndAcquireGPUSwapchainTexture(
                g_cmd, g_window, &g_swapchain, NULL, NULL);
        }
    ...
    {
        GPUTextureEntry *scene = TextureEntry(g_scene_texture_id);
                                    ...
                                    int height, int clear)
{
    ...
    g_target_color = tex ? tex->texture : NULL;
    g_target_color_format = tex ? tex->format : SDL_GPU_TEXTUREFORMAT_R8G8B8A8_UNORM;
...
MikuPan_GPUUniformBlock *MikuPan_GPUUniforms(void) { return &g_uniforms; }
MikuPan_LightData *MikuPan_GPULightData(void) { return &g_light_data; }
MikuPan_MaterialData *MikuPan_GPUMaterialData(void) { return &g_material_data; }

static void CopyMatrix3Std140(float out[12], const float *mat)
{
    out[0] = mat[0]; out[1] = mat[1]; out[2] = mat[2]; out[3] = 0.0f;
    out[4] = mat[3]; out[5] = mat[4]; out[6] = mat[5]; out[7] = 0.0f;
    out[8] = mat[6]; out[9] = mat[7]; out[10] = mat[8]; out[11] = 0.0f;
}

void MikuPan_GPUSetMatrix4(const char *name, const float *mat)
{
    if (strcmp(name, "model") == 0) memcpy(g_uniforms.model, mat, sizeof(g_uniforms.model));
    else if (strcmp(name, "view") == 0) memcpy(g_uniforms.view, mat, sizeof(g_uniforms.view));
    else if (strcmp(name, "projection") == 0) memcpy(g_uniforms.projection, mat, sizeof(g_uniforms.projection));
...
{
    if (strcmp(name, "uColor") == 0) memcpy(g_uniforms.uColor, vec, sizeof(g_uniforms.uColor));
    else if (strcmp(name, "uFog") == 0) memcpy(g_uniforms.uFog, vec, sizeof(g_uniforms.uFog));
...
{
    if (strcmp(name, "renderNormals") == 0) g_uniforms.uFlags0[0] = value;
    else if (strcmp(name, "disableLighting") == 0) g_uniforms.uFlags0[1] = value;
    else if (strcmp(name, "staticLighting") == 0) g_uniforms.uFlags0[2] = value;
    else if (strcmp(name, "uMeshLightingMode") == 0) g_uniforms.uFlags0[3] = value;
    else if (strcmp(name, "uMirrorSurfacePass") == 0) g_uniforms.uFlags1[0] = value;
    else if (strcmp(name, "uShadowEnabled") == 0) g_uniforms.uFlags1[1] = value;
    else if (strcmp(name, "uShadowDebugView") == 0) g_uniforms.uFlags1[2] = value;
    else if (strcmp(name, "uCrtEnabled") == 0) g_uniforms.uFlags1[3] = value;
    else if (strcmp(name, "uBlackWhiteMode") == 0) g_uniforms.uFlags2[0] = value;
    else if (strcmp(name, "uPhotoNegativeEnabled") == 0) g_uniforms.uFlags2[1] = value;
...
{
    if (strcmp(name, "uNormalLength") == 0) g_uniforms.uParams0[0] = value;
    else if (strcmp(name, "uShadowStrength") == 0) g_uniforms.uParams0[1] = value;
    else if (strcmp(name, "uBrightness") == 0) g_uniforms.uParams0[2] = value;
    else if (strcmp(name, "uGamma") == 0) g_uniforms.uParams0[3] = value;
    else if (strcmp(name, "uCrtStrength") == 0) g_uniforms.uCrt0[0] = value;
    else if (strcmp(name, "uCrtCurvature") == 0) g_uniforms.uCrt0[1] = value;
    else if (strcmp(name, "uCrtOverscan") == 0) g_uniforms.uCrt0[2] = value;
    else if (strcmp(name, "uCrtScanlineStrength") == 0) g_uniforms.uCrt0[3] = value;
    else if (strcmp(name, "uCrtScanlineScale") == 0) g_uniforms.uCrt1[0] = value;
    else if (strcmp(name, "uCrtScanlineThickness") == 0) g_uniforms.uCrt1[1] = value;
    else if (strcmp(name, "uCrtMaskStrength") == 0) g_uniforms.uCrt1[2] = value;
    else if (strcmp(name, "uCrtMaskScale") == 0) g_uniforms.uCrt1[3] = value;
    else if (strcmp(name, "uCrtVignetteStrength") == 0) g_uniforms.uCrt2[0] = value;
    else if (strcmp(name, "uCrtVignetteSize") == 0) g_uniforms.uCrt2[1] = value;
    else if (strcmp(name, "uCrtChromaOffset") == 0) g_uniforms.uCrt2[2] = value;
    else if (strcmp(name, "uCrtBlendStrength") == 0) g_uniforms.uCrt2[3] = value;
    else if (strcmp(name, "uCrtBlendRadius") == 0) g_uniforms.uCrt3[0] = value;
    else if (strcmp(name, "uCrtNoiseStrength") == 0) g_uniforms.uCrt3[1] = value;
    else if (strcmp(name, "uCrtFlickerStrength") == 0) g_uniforms.uCrt3[2] = value;
...
void MikuPan_GPUSetVec2(const char *name, float x, float y)
{
    ...
    {
        g_uniforms.uTextureSize[0] = x; g_uniforms.uTextureSize[1] = y;
    }
    else if (strcmp(name, "uOutputSize") == 0)
    {
        g_uniforms.uOutputSize[0] = x; g_uniforms.uOutputSize[1] = y;
    }
    else if (strcmp(name, "uShadowSize") == 0)
    {
        g_uniforms.uShadowSize[0] = x; g_uniforms.uShadowSize[1] = y;
    }
    else if (strcmp(name, "uFramebufferUvOffset") == 0)
    {
        g_uniforms.uFramebufferUvOffset[0] = x; g_uniforms.uFramebufferUvOffset[1] = y;
    }
    else if (strcmp(name, "uFramebufferUvScale") == 0)
    {
        g_uniforms.uFramebufferUvScale[0] = x; g_uniforms.uFramebufferUvScale[1] = y;
    }
    else if (strcmp(name, "uFramebufferContentUvMax") == 0)
    {
        g_uniforms.uFramebufferContentUvMax[0] = x; g_uniforms.uFramebufferContentUvMax[1] = y;
    }
