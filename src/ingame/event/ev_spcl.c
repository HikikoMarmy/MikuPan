...
void StarPuzzleDisp()
{
    ...
    for (i = 0; i < 11; i++)
    {
        SimpleDispSprt(&spev00_sp_bak[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i, NULL, NULL, alp_rate);
    }
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        )
        {
            if (star_pzl_wrk.line[star_pzl_wrk.slct_no][star_pzl_wrk.empty] == i)
            {
                ...
                SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 14, &spev00_lin_rot[i], &spev00_lin_scl[i], alp_rate);
            }
        }
    }
    ...
    for (i = 0; i < 10; i++)
    {
        if (star_pzl_wrk.mode != STAR_PZL_MODE_GOUT && star_pzl_wrk.mode != STAR_PZL_MODE_BAD && star_pzl_wrk.mode != STAR_PZL_MODE_OUT)
        {
            if (star_pzl_dat[star_pzl_wrk.pzl_no].line[i] != 0)
            {
                SimpleDispSprt(spev00_sp_lia + i, MikuPan_GetHostAddress(EVENT_ADDRESS), 14, &spev00_lia_rot[i], &spev00_lia_scl[i], alp_rate);
            }
        }
    }
    ...
    for (i = 0; i < 5; i++)
    {
        if (star_pzl_wrk.mode == STAR_PZL_MODE_READYGOOD2 || star_pzl_wrk.mode == STAR_PZL_MODE_GOOD || star_pzl_wrk.mode == STAR_PZL_MODE_GOUT)
        {
            ...
            SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 13, NULL, NULL, alp_rate);
            ...
            if (star_pzl_wrk.mode == STAR_PZL_MODE_GOOD)
            {
                ...
                SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 14, NULL, NULL, alp_rate);
            }
        }
    ...
    {
        SimpleDispSprt(&spev00_sp_lia[star_pzl_wrk.empty + 10], MikuPan_GetHostAddress(EVENT_ADDRESS), 14, NULL, NULL, alp_rate);
        SimpleDispSprt(&spev00_sp_lia[star_pzl_wrk.slct_no + 10], MikuPan_GetHostAddress(EVENT_ADDRESS), 14, NULL, NULL, alp_rate);
    }

    SimpleDispSprt(&spev00_sp_hln[star_pzl_wrk.empty], MikuPan_GetHostAddress(EVENT_ADDRESS), 16, NULL, NULL, alp_rate);
    SimpleDispSprt(&spev00_sp_hln[star_pzl_wrk.slct_no], MikuPan_GetHostAddress(EVENT_ADDRESS), 16, NULL, NULL, alp_rate);

    if (star_pzl_wrk.mode == STAR_PZL_MODE_MOVE)
    {
        SimpleDispSprtDatCopy(&spev00_sp_num[star_pzl_wrk.count-1], &ssd);
        ssd.alp = ssd.alp * (30 - star_pzl_wrk.time) / 30;
        SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 15, NULL, NULL, alp_rate);
    ...
    {
        SimpleDispSprt(&spev00_sp_num[star_pzl_wrk.count], MikuPan_GetHostAddress(EVENT_ADDRESS), 15, NULL, NULL, alp_rate);
    }
        ...
        for (i = 0; i < 5; i++)
        {
            if (star_pzl_wrk.stone[star_pzl_wrk.slct_no] == i)
            {
                if (star_pzl_wrk.mode == STAR_PZL_MODE_START)
                {
                    ...
                    ssd.alp = ssd.alp * (40 - star_pzl_wrk.time) / 40;
                    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, alp_rate);
                }
                else
                {
                    SimpleDispSprt(&spev00_sp_bff[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, alp_rate);
                }
            }
        }
    }
    ...
    if (star_pzl_wrk.mode != STAR_PZL_MODE_IN && star_pzl_wrk.mode != STAR_PZL_MODE_OUT)
    {
        for (i = 0; i < 5; i++)
        {
            if (star_pzl_wrk.mode == STAR_PZL_MODE_START)
            {
                ...
                ssd.alp = ssd.alp * (40 - star_pzl_wrk.time) / 40;
                SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, NULL, alp_rate);
            }
            else
            {
                SimpleDispSprt(&spev00_sp_bft[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, NULL, alp_rate);
            }
        }
    }
    ...
    if (star_pzl_wrk.mode == STAR_PZL_MODE_MENU)
    {
        ...
        if (star_pzl_wrk.menu_mode == STAR_PZL_MENU_SLCT)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                ...
                if (i < 2)
                {
                    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 17, NULL, NULL, 0x64);
                }
                else
                {
                    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 17, NULL, &spev00_csr_scl[0], 0x64);
                }
            }
        }
        else if (star_pzl_wrk.menu_mode == STAR_PZL_MENU_RESET)
        {
            ...
            for (i = 3; i < 6; i++)
            {
                ...
                {
                    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 17, NULL, &spev00_csr_scl[star_pzl_wrk.menu_csr[1] + 1], 0x64);
                }
            }
        }
        else if (star_pzl_wrk.menu_mode == STAR_PZL_MENU_EXIT)
    ...
    SimpleDispSprt(&spev00_sp_cpf[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, alp_rate);
    SimpleDispSprt(&spev00_sp_cpf[2], MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, alp_rate);
#ifdef BUILD_EU_VERSION
    SimpleDispSprt(&spev00_sp_cpf[4], MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, alp_rate);
#endif

    if (star_pzl_wrk.count < star_pzl_dat[star_pzl_wrk.pzl_no].move_num)
    {
        SimpleDispSprt(&spev00_sp_cpf[1], MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, alp_rate);
    }
    else
    {
        SimpleDispSprt(&spev00_sp_cpf[3], MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, alp_rate);
    }

    if (star_pzl_wrk.count != 0)
    {
        SimpleDispSprt(&spev00_sp_cpf2[8], MikuPan_GetHostAddress(EVENT_ADDRESS), 20, NULL, NULL, alp_rate);
        SimpleDispSprt(&spev00_sp_cpf2[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 20, NULL, NULL, alp_rate);
        SimpleDispSprt(&spev00_sp_cpf2[star_pzl_wrk.count], MikuPan_GetHostAddress(EVENT_ADDRESS), 20, NULL, NULL, alp_rate);
    }
    else
    {
        SimpleDispSprt(&spev00_sp_cpf2[8], MikuPan_GetHostAddress(EVENT_ADDRESS), 20, NULL, NULL, alp_rate);
        SimpleDispSprt(&spev00_sp_cpf2[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 20, NULL, NULL, alp_rate);
        SimpleDispSprt(&spev00_sp_cpf2[1], MikuPan_GetHostAddress(EVENT_ADDRESS), 20, NULL, NULL, alp_rate);
    }

    SimpleDispSprt(spev00_sp_cap, MikuPan_GetHostAddress(EVENT_ADDRESS), 19, NULL, NULL, alp_rate);
...
void DialKeyDoorDisp()
{
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 16, NULL, &sscl, alp_rate);
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, alp_rate);
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 19, NULL, NULL, alp_rate);
...
void DialKeyMSGDoorDisp(int msg_no)
{
    ...
    for (i = 0; i < 2; i++)
    {
        SimpleDispSprt(&spev01_sp_gl3[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, NULL, alp_rate);
    }
...
void GhostDoorDisp(int door_no)
{
    ...
    if (door_no != 5)
    {
        for (i = 0; i < 11; i++)
        {
            SimpleDispSprt(&spev02_sp_bak[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i, NULL, NULL, 100);
        }

        if (spev_wrk.count != 0)
        {
            SimpleDispSprt(&spev02_sp_of1[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 100);
        }
    }
    else
    {
...
void SpecialEventDisp003(int no)
{
    ...
    for (i = 0; i < 11; i++)
    {
        SimpleDispSprt(&spev03_sp_bak[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i, NULL, NULL, 0x64);
    }
            ...
            if (i < 6)
            {
                if (spev_wrk.csr[0] == i)
                {
                    if (spev_wrk.csr[0] < 6)
                    {
                        ...
                        {
                            SimpleDispSprt(&ssd_csr, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, &spev03_dol_left, &spev03_dla_scl[i], 0x64);
                        }
                        else
                        {
                            SimpleDispSprt(&ssd_csr, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, &spev03_dla_scl[i], 0x64);
                        }
                    }
                    else
                    {
                        if (evdl_dat[no].order[i] == 1)
                        {
                            SimpleDispSprt(&ssd_csr, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, &spev03_dol_left, NULL, 0x64);
                        }
                        else
                        {
                            SimpleDispSprt(&ssd_csr, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, NULL, 0x64);
                        }
                    }
                }
                ...
                if (evdl_dat[no].order[i] == 1)
                {
                    SimpleDispSprtRGB(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 11, &spev03_dol_left, &spev03_dol_scl[i], 0x64, ssd_tmp_col, ssd_tmp_col, ssd_tmp_col);
                }
                else
                {
                    SimpleDispSprtRGB(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, &spev03_dol_scl[i], 0x64, ssd_tmp_col, ssd_tmp_col, ssd_tmp_col);
                }
            }
            else
            {
                ...
                if (evdl_dat[no].order[i] == 1)
                {
                    SimpleDispSprtRGB(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 11, &spev03_dol_left, NULL, 0x64, ssd_tmp_col, ssd_tmp_col, ssd_tmp_col);
                }
                else
                {
                    SimpleDispSprtRGB(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64, ssd_tmp_col, ssd_tmp_col, ssd_tmp_col);
                }
            }
        }
    }
    ...
    SimpleDispSprt(&spev03_sp_cap[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64);
#ifdef BUILD_EU_VERSION
    SimpleDispSprt(&spev03_sp_cap[1], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64);
    SimpleDispSprt(&spev03_sp_cap[2], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64);
    SimpleDispSprt(&spev03_sp_cap[3], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64);
...
void SpecialEventDisp004()
{
    ...
    for (i = 0; i < 3; i++)
    {
        SimpleDispSprt(&spev04_sp_bk0[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i, NULL, NULL, 0x64);
    }

    for (i = 0; i < 3; i++)
    {
        SimpleDispSprt(&spev04_sp_bk1[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 3, NULL, NULL, 0x64);
    }

    for (i = 0; i < 3; i++)
    {
        SimpleDispSprt(&spev04_sp_bk2[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 4, NULL, NULL, 0x64);
    }

    for (i = 0; i < 3; i++)
    {
        SimpleDispSprt(&spev04_sp_bk3[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i + 5, NULL, NULL, 0x64);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        if (evbtz_dat[i].use_flg == 0)
        {
            SimpleDispSprt(&spev04_sp_bd1[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 8, NULL, &spev04_bd1_scl[i], 0x64);
        }
        else
        {
            ...
            SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, NULL, &spev04_bz2_scl[evbtz_dat[i].set_place], 0x64);
        }
    }
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, NULL, NULL, 0x64);
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, &spev03_dol_left, NULL, 0x64);
    ...
    if (spev_wrk.mode == 5)
    {
        ...
        {
            SimpleDispSprt(&ssd3, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, NULL, &spev04_sel_scl2[spev_wrk.csr[1] >> 1], 0x64);
            SimpleDispSprt(&ssd4, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, NULL, &spev04_sel_scl2[spev_wrk.csr[1] >> 1], 0x64);
        }
        else
        {
            SimpleDispSprt(&ssd3, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, &spev03_dol_left, &spev04_sel_scl2[spev_wrk.csr[1] >> 1], 0x64);
            SimpleDispSprt(&ssd4, MikuPan_GetHostAddress(EVENT_ADDRESS), 8, &spev03_dol_left, &spev04_sel_scl2[spev_wrk.csr[1] >> 1], 0x64);
        }
    }
    ...
    for (i = 0; i < 3; i++)
    {
        SimpleDispSprt(&spev04_sp_bd2[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 9, NULL, NULL, 0x64);
    }
...
#endif
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 10, NULL, NULL, 0x64);
...
void SpecialEventDisp014(int no)
{
    ...
    for (i = 0; i < 11; i++)
    {
        SimpleDispSprt(&spev20_sp_bak[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i, NULL, NULL, 0x64);
    }
    ...
    for (i = 0; i < 6; i++)
    {
        if (spev_wrk.csr[0] == i)
        {
            ...
            SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, NULL, 0x64);
        }
    }
    ...
    if (evcdl_dat.stflame_shape[0] != -1)
    {
        for (i = 0; i < 6; i++)
        {
            ...
            SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 14, NULL, NULL, 0x64);
        }
    }
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 13, NULL, NULL, 0x64);
    }
    ...
    SimpleDispSprt(&spev20_sp_cpf[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64);
    SimpleDispSprt(&spev20_sp_cdl5[0], MikuPan_GetHostAddress(EVENT_ADDRESS), 15, NULL, NULL, 0x64);
...
void FaceDoorDisp(int face_no)
{
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS_2), 0, NULL, NULL, 0x64);
    ...
    else if (face_no == 3)
    {
        ...
        ssd.alp = spev_wrk.csr[1];
        SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS_2), 4, NULL, NULL, 0x64);
    }
...
void SurpriseDoorDisp(int face_no)
{
    ...
    SimpleDispSprt(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS_2), 5, NULL, NULL, 0x64);
...
void ZushiBonjiDisp(int bonji_no)
{
    ...
    for (i = 0; i < 11; i++)
    {
        SimpleDispSprtLNR(&spev21_sp_bak[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i, NULL, NULL, 0x64, 0);
    }
    ...
    if (spev_wrk.mode == 7 || spev_wrk.mode == 5 || spev_wrk.mode == 8 || spev_wrk.mode == 11)
    {
        for (i = 0; i < 4; i++)
        {
            if (i == spev_wrk.csr[0])
            {
                SimpleDispSprtLNR(&spev21_sp_zbt[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i + 13, NULL, NULL, 0x64, 1);
            }
        }
    }
    ...
    for (i = 0; i < 4; i++)
    {
        if (i == spev_wrk.csr[0])
        {
            ...
            SimpleDispSprtLNR(&ssd, MikuPan_GetHostAddress(EVENT_ADDRESS), 12, NULL, NULL, 0x64, 0);
        }
    }
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
            SimpleDispSprtLNR(&spev21_sp_zft[i+4], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64, 0);
        }
        else
        {
            SimpleDispSprtLNR(&spev21_sp_zft[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64, 0);
        }
    }
    ...
    for (i = 0; i < 1; i++)
    {
        SimpleDispSprtLNR(&spev21_sp_cpf[i], MikuPan_GetHostAddress(EVENT_ADDRESS), 17, NULL, NULL, 0x64, 0);
    }

    SimpleDispSprtLNR(spev21_sp_cap, MikuPan_GetHostAddress(EVENT_ADDRESS), 18, NULL, NULL, 0x64, 0);
...
void ZushiBonjiAfterDisp(int bonji_no)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (i == spev_wrk.csr[1])
        {
            SimpleDispSprtLNR(&spev21_sp_zbt[i], MikuPan_GetHostAddress(EVENT_ADDRESS), i + 13, NULL, NULL, 0x64, 0);
            SimpleDispSprtLNR(&spev21_sp_zft[i+4], MikuPan_GetHostAddress(EVENT_ADDRESS), 11, NULL, NULL, 0x64, 0);
        }
        else
        {
