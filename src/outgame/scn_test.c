...
int SceneTestLoad()
{
    ...
    {
    case 0:
        SceneDataLoadReq(scn_test_wrk.scene_no, (u_int *)MikuPan_GetHostAddress(SCENE_LOAD_ADDRESS));
    ...
    case 2:
        if (IsLoadEnd(load_id) != 0)
        {
            if (scn_p->chapter_no < 0)
            {
                RoomMdlLoadReq((u_int *)MikuPan_GetHostAddress(MODEL_DATA_ADDRESS), 0, 3, scn_p->room_no, 0);
            }
            else
            {
                RoomMdlLoadReq((u_int *)MikuPan_GetHostAddress(MODEL_DATA_ADDRESS), 0, scn_p->chapter_no, scn_p->room_no, 0);
            }
    ...
    case 4:
        if (MsnInitPlyr() != 0)
        {
            ScnTestMdlLoadReq((u_int *)MikuPan_GetHostAddress(SCENE_TEST_MD_LOAD_ADDRESS));
